...
async function getV4QuestionsFallback(commune) {
  ...
  const questionsData = JSON.parse(
    await readFile(join(__dirname, '..', '..', 'data', 'questions.json'), 'utf-8')
  );
...
export default async function handler(req, res) {
  ...
  const { commune: communeCode } = req.query;

  if (!communeCode) {
    return res.status(400).json({ success: false, error: 'Code commune manquant' });
  }
