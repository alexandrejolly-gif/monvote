...
const CACHE_VERSION = 'v4'; // Incrémenter pour forcer la réinitialisation du cache
const CACHE_VERSION_KEY = 'geojson_cache_version';

/**
 * Vérifie la version du cache et le vide si nécessaire
 */
function checkCacheVersion() {
  const storedVersion = localStorage.getItem(CACHE_VERSION_KEY);
  if (storedVersion !== CACHE_VERSION) {
    console.log(`🔄 Cache version outdated (${storedVersion || 'none'} → ${CACHE_VERSION}), clearing...`);
    clearGeometryCache();
...
export async function fetchCommuneGeometry(communeCode) {
  // 0. Vérifier la version du cache
  checkCacheVersion();
  ...
  try {
    ...
    console.log(`📍 Fetched geometry for code ${communeCode}: got ${data.nom} (${data.code})`);
